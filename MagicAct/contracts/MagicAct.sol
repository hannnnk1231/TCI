...
contract MagicAct is Audience {
	...
	function cardTrick(uint _amount) public checkDonation(100) {
	...
	function partyShow(uint _amount) public checkDonation(1000) {
		require(audience[msg.sender].cardTrickCooldown <= now);
	...
	function stageShow(uint _amount) public checkDonation(5000) {
		require(audience[msg.sender].cardTrickCooldown <= now);
	...
	function illution(uint _amount) public checkDonation(10000) {
		require(audience[msg.sender].cardTrickCooldown <= now);
